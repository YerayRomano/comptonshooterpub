...
int main(int argc,char **argv)
{
	...
	if(jpg == 1) {
	  ...
	  SDL_WM_SetIcon(IMG_Load("crip.jpg"),NULL);
	}
	if(!tela) {
	...
	dollaz= 30;
	cwalk.x = 0;
	cwalk.y = tela->h -dollaz;
	cwalk.w = tela->w;
	cwalk.h = dollaz;					
	SDL_FillRect(tela,&cwalk,SDL_MapRGB(tela->format,128,128,128));
	SDL_UpdateRects(tela,1,&cwalk);
	if(jpg == 1) {
	  mapa0 = IMG_Load("ladrillo.jpg");
	} else {
	...
	if(jpg == 1) {
	  bote0 = IMG_Load("botella.jpg");
	} else {
	  bote0 = SDL_LoadBMP("botella.bmp");
	}
	if(jpg == 1) {
	  bote1 = IMG_Load("botella.jpg");
	} else {
	...
	if(tempo <= 9) {
		if(jpg == 1) {
		  minuto = IMG_Load("puntua.jpg");
		  sprintf(&class,"%d.jpg",tempo);
		  wrekin = IMG_Load(class);
		} else {
		  minuto = SDL_LoadBMP("puntua.bmp");
		  sprintf(&class,"%d.bmp",tempo);
		  wrekin = SDL_LoadBMP(class);
		}
		if(!minuto) {
		...
		if(!wrekin) {
		    if(tempo != 0) {
			printf("algun jacoso se llevo el marcador al cash converters\n");
		    }
		}
		if(tempo == 0) {
		  SDL_FreeSurface(minuto);
		  if(jpg == 1) {
		      minuto = IMG_Load("tramposo.jpg");
		  } else {
		      minuto = SDL_LoadBMP("tramposo.bmp");
		  }
		  if(!minuto) {
		      printf("eres un tramposo\n");
		  }
		  ...
		  SDL_Delay(3000);
		    ...
		    world.w = wrekin->w;
		    world.h = wrekin->h;
		    SDL_BlitSurface(wrekin,NULL,tela,&world);
		    SDL_UpdateRects(tela,1,&world);
		    SDL_Delay(300);
		}
	} else {
		if(jpg == 1) {
		  minuto = IMG_Load("tupac.jpg");
		} else {
