...
int main(int argc,char **argv)
{
	...
	inito:
	null = SDL_GetVideoSurface();	
	menu = SDL_LoadBMP("eusebio.bmp");
	if(!menu) {
	...
	SDL_Delay(3000);
	SDL_BlitSurface(null,NULL,tela,&estampa);
	SDL_UpdateRects(tela,1,&estampa);	
	...
	mapa2 = SDL_LoadBMP("piso.bmp");
	if(!mapa2) {
        printf("donde esta el piso de porcelana china\n");
        exit(-1);   
    }
    fijo2.x = 250;
	fijo2.y = menu->h - mapa2->h - suelo->h;
	fijo2.w = mapa2->w;
	fijo2.h = mapa2->h;
	SDL_BlitSurface(mapa2,NULL,tela,&fijo2);
	SDL_UpdateRects(tela,1,&fijo2);
	x_2 = rand() % 400 + 300;
	printf("x_2=%d\n",x_2);
	fijo1.x = x_2;
	...
	if(!bote2) {
        priintf("un borracho se ha bebido las botellas\n");
        SDL_FreeSurface(tela);    
    }
	...
	botella2.w = fijo1.w;
	botella2.h = fijo1.h;
	botella3.x = fijo2.x + 8;
	botella3.y = fijo2.y - bote1->h;
	botella3.w = fijo2.w;
	botella3.h = fijo2.h;
	...
	while(cont <= 1) {
		...
		switch(juego.type) {
			...
			case SDL_MOUSEBUTTONDOWN:
				...
				if(y_raton >= botella3.y) {
					if(y_raton <= (botella3.y + bote0->h)) {					
						if(x_raton >= botella3.x) {						
							if(x_raton <= (botella3.x + bote0->w)) {							
								printf("tocado\n");
								cont++;
								actualizar = 1;
								tres = 3;
				...
				if(actualizar == 1) {
					...
					if(tres == 0) {
						SDL_BlitSurface(bote2,NULL,tela,&botella3);
						SDL_UpdateRects(tela,1,&botella3);					
					}
					actualizar = 0;				
				}
			break;
